...
import numpy as np
import pandas as pd
from brl.Entropy import compute_entropy, cut_point_information_gain
...
class MDLP_Discretizer(object):
    def __init__(
        ...
        self._X_raw = X  # copy or original input data
        self._class_name = class_label
        self._classes = self._X_raw[self._class_name]  # .unique()
        self._classes.drop_duplicates()

        # if user specifies which attributes to discretize
        if features:
            self._features = [
                f for f in features if f in self._X_raw.columns
            ...
            if missing:
                print(
                    f"WARNING: user-specified features {missing} not in input dataframe"
                )
        else:  # then we need to recognize which features are numeric
            numeric_cols = self._X_raw._data.get_numeric_data().items
            self._features = [f for f in numeric_cols if f != class_label]

        # other features that won't be discretized
        self._ignored_features = set(self._X_raw.columns) - set(self._features)

        # create copy of data only including features to discretize and class
        self._data = self._X_raw.loc[:, self._features + [class_label]]
        for c in self._data.columns:
            self._data[c] = pd.to_numeric(self._data[c])
        ...
        self._cuts: dict[str, list] = {f: [] for f in self._features}
    ...
    def MDLPC_criterion(self, data, feature, cut_point) -> bool:
        """
        ...
        partition_entropy = compute_entropy(data_partition[self._class_name])
        ...
        entropy_left = compute_entropy(
            data_left[self._class_name]
        )  # entropy of partition
        entropy_right = compute_entropy(data_right[self._class_name])
        delta = (
            log(3**k, 2)
            - (k * partition_entropy)
            + (k_left * entropy_left)
    ...
    def feature_boundary_points(self, data, feature):
        ...
        potential_cuts = data_partition[data_partition["feature_change"]].index[1:]
        sorted_index = data_partition.index.tolist()

        for row in potential_cuts:
            old_value = data_partition.loc[sorted_index[sorted_index.index(row) - 1]][
                feature
            ]
    ...
    def compute_boundary_points_all_features(self) -> dict[str, Any]:
        """
    ...
    def boundaries_in_partition(self, data, feature):
        ...
        return set(
            [
                x
                for x in self._boundaries[feature]
    ...
    def apply_cutpoints(self, out_data_path=None, out_bins_path=None):
        ...
        for attr in self._features:
            if len(self._cuts[attr]) == 0:
                ...
                bin_labels = [f"{cuts[i]}_to_{cuts[i + 1]}" for i in start_bin_indices]
        ...
        if self._ignored_features:
            to_return = pd.concat(
                [self._data, self._X_raw[list(self._ignored_features)]], axis=1
            )
            to_return = to_return[
                self._X_raw.columns
        ...
        if out_bins_path:
            with open(out_bins_path, "w") as bins_file:
                ...
                for attr in self._features:
                    print(
                        f"attr: {attr}\n\t{', '.join([bin_label for bin_label in bin_label_collection[attr]])}",
                        file=bins_file,
                    )


def main(argv):
    out_path_data, out_path_bins, return_bins, class_label, features = (
        None,
        None,
        False,
        None,
    ...
    try:
        ...
        print(
            f"main - Correct usage: python MDLP.py --in_path=path --out_path=path --features=f1,f2,f3... --class_label=weather --return_bins\n{e}"
        )
        sys.exit(2)
    ...
    # read input data
    assert data_path, f"main - {data_path=}"
