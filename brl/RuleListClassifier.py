...
from brl.Discretization.MDLP import MDLP_Discretizer
from brl.LethamBRL.BRL_code import (
    default_permsdic,
    fpgrowth,
    get_point_estimate,
    get_rule_rhs,
...
class RuleListClassifier(BaseEstimator):
    ...
    def __init__(
        ...
        self.burnin = (
            self.max_iter // 2
        )  # the number of samples to drop as burn-in in-simulation
    ...
    def _setlabels(self, X, feature_labels=[]):
        if len(feature_labels) == 0:
            if isinstance(X, pd.DataFrame) and (
                ...
                feature_labels = [f"ft{i + 1}" for i in range(len(X[0]))]
    ...
    def _discretize_mixed_data(self, X, y, undiscretized_features=[]):
        ...
        for fi in range(len(X[0])):
            ...
            if isinstance(X[0][fi], numbers.Number) and (
                len(self.feature_labels) == 0
                or len(undiscretized_features) == 0
                or self.feature_labels[fi] not in undiscretized_features
            ):
    ...
    def fit(self, X, y, feature_labels=[], undiscretized_features=[]):
        ...
        Xtrain, Ytrain, nruleslen, lhs_len, self.itemsets = (
            X,
            np.vstack((1 - np.array(y), y)).T.astype(int),
            nruleslen,
            lhs_len,
            itemsets_all,
        )
        ...
        if self.d_star:
            ...
            self.theta, self.ci_theta = get_rule_rhs(
                Xtrain, Ytrain, self.d_star, self.alpha, True
            )
    ...
    def discretize(self, X, y):
        ...
        D = pd.DataFrame(
            np.hstack((X, np.array(y).reshape((len(y), 1)))),
            columns=list(self.feature_labels) + ["y"],
        )
        self.discretizer = MDLP_Discretizer(
            X=D, class_label="y", features=self.discretized_features
        )
        ...
        for i in range(len(self.feature_labels)):
            ...
            if label in self.discretized_features:
                ...
                for j in range(len(self.discretizer._data[label])):
                    column += [f"{label} : {self.discretizer._data[label][j]}"]
    ...
    def _prepend_feature_labels(self, X):
        ...
        for i in range(len(Xl)):
            for j in range(len(Xl[0])):
                Xl[i][j] = f"{self.feature_labels[j]} : {Xl[i][j]}"
        return Xl
    ...
    def tostring(self, decimals=1):
        if self.d_star:
            ...
            if self.class1label != "class 1":
                detect = f"for detecting {self.class1label}"
            header = f"Trained RuleListClassifier {detect}\n"

            separator = "".join(["="] * len(header)) + "\n"
            s = ""
            for i, j in enumerate(self.d_star):
                if self.itemsets[j] != "null":
                    condition = f"ELSE IF {' AND '.join([str(self.itemsets[j][k]) for k in range(len(self.itemsets[j]))])} THEN"
                else:
                    condition = "ELSE"
                s += f"{condition} probability of {self.class1label}: {np.round(self.theta[i] * 100, decimals)}% ({np.round(self.ci_theta[i][0] * 100, decimals)}%-{np.round(self.ci_theta[i][1] * 100, decimals)}%)\n"
